...
const sourceColor: Record<string, string> = {
  'HR System': 'bg-purple-50 text-purple-700 dark:bg-purple-950/30 dark:text-purple-300',
  'Signup Form': 'bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-300',
  'Directory': 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300',
};
  ...
  return (
    <div className="p-6 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Volunteer Matching</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Skill-based and interest-based matching across all registered volunteers</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="secondary" size="md" icon={<Upload size={14} />} onClick={() => setShowCsvModal(true)}>
            Import CSV
          </Button>
          <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg px-3 py-2 text-xs text-blue-700 dark:text-blue-300">
            <Zap size={13} />
      ...
      <div className="grid grid-cols-5 gap-5">
        ...
        <div className="col-span-2 space-y-3">
          <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest px-1">Open Opportunities</p>
          {opportunities.map(opp => (
            <Card
              key={opp.id}
              onClick={() => setSelectedOppId(opp.id)}
              className={`transition-all ${selectedOppId === opp.id ? 'border-blue-400 shadow-md ring-1 ring-blue-200 dark:ring-blue-800' : ''}`}
            >
              <div className="p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-100 leading-snug">{opp.title}</h4>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mb-3 line-clamp-2">{opp.description}</p>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {opp.topics.map(t => (
                    <span key={t} className="text-xs bg-indigo-50 dark:bg-indigo-950/30 text-indigo-700 border border-indigo-100 dark:border-indigo-800 rounded-full px-2 py-0.5">{t}</span>
                  ))}
                </div>
        ...
        <div className="col-span-3">
          <Card>
            ...
            {/* Skills needed */}
            <div className="px-5 py-3 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700 flex flex-wrap gap-1.5 items-center">
              <span className="text-xs font-medium text-slate-500 dark:text-slate-400 mr-1">Required skills:</span>
              {selectedOpp.requiredSkills.map(s => (
                <span key={s} className="text-xs bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 rounded-full px-2 py-0.5">{s}</span>
              ))}
            </div>

            <div className="divide-y divide-slate-50 dark:divide-slate-700">
              {matches.map((v, i) => (
                <div key={v.id} className={`p-5 ${i === 0 ? 'bg-emerald-50/40 dark:bg-emerald-950/30' : ''}`}>
                  <div className="flex items-start gap-4">
                    {/* Rank */}
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                      i === 0 ? 'bg-emerald-500 text-white' : i === 1 ? 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-200' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'
                    }`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-100">{v.name}</h4>
                        {i === 0 && <span className="text-xs bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300 rounded-full px-2 py-0.5 font-medium">Best match</span>}
                      ...
                      {v.matchReasons.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {v.matchReasons.map(r => (
                            <span key={r} className="text-xs bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-800 rounded-full px-2.5 py-0.5 flex items-center gap-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {v.skills.map(s => {
                          ...
                          return (
                            <span key={s} className={`text-xs rounded-full px-2 py-0.5 ${isMatch ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 font-medium dark:bg-emerald-950/30 dark:text-emerald-300 dark:border-emerald-800' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'}`}>
                              {s}
                            </span>
                          );
                        })}
                    ...
                    <div className="shrink-0">
                      {isAssigned(v.id) ? (
                        <div className="flex items-center gap-1.5 text-xs text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-lg px-3 py-2">
                  ...
                  {v.matchScore < 40 && (
                    <div className="mt-3 ml-11 flex items-center gap-1.5 text-xs text-amber-600 bg-amber-50 dark:bg-amber-950/30 rounded-lg px-3 py-2">
